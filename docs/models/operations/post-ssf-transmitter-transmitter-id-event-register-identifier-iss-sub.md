# PostSsfTransmitterTransmitterIdEventRegisterIdentifierIssSub

The subject identifier with the `iss_sub` format as defined in [Section 3.2.3. Issuer and Subject Identifier Format](https://www.rfc-editor.org/rfc/rfc9493.html#section-3.2.3) of [RFC 9493: Subject Identifiers for Security Event Tokens](https://www.rfc-editor.org/rfc/rfc9493.html).


## Example Usage

```typescript
import { PostSsfTransmitterTransmitterIdEventRegisterIdentifierIssSub } from "@authlete/arena/models/operations";

let value: PostSsfTransmitterTransmitterIdEventRegisterIdentifierIssSub = {
  format: "iss_sub",
  iss: "<value>",
  sub: "<value>",
};
```

## Fields

| Field              | Type               | Required           | Description        |
| ------------------ | ------------------ | ------------------ | ------------------ |
| `format`           | *"iss_sub"*        | :heavy_check_mark: | N/A                |
| `iss`              | *string*           | :heavy_check_mark: | N/A                |
| `sub`              | *string*           | :heavy_check_mark: | N/A                |