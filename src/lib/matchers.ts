...
import { ArenaDefaultError } from "../models/errors/arena-default-error.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | ArenaDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | ArenaDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new ArenaDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new ArenaDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
